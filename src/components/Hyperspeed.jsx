...
const Hyperspeed = ({
  ...
  // Handle fade in/out effect based on context isPressed
  useEffect(() => {
    if (isPressed) {
      setIsVisible(true);
    } else {
      setIsVisible(false); // Immediate fade out when released
    }
  }, [isPressed]);
  ...
  useEffect(() => {
    ...
    class App {
      constructor(container, options = {}) {
        ...
        this.speedUp = 0;
        this.timeOffset = 0;
        this.externalIsPressed = options.isPressed || false;
      ...
      update(delta) {
        // Update external isPressed state
        this.externalIsPressed = this.options.isPressed || false;
        
        // Set speed and FOV based on external state
        if (this.externalIsPressed) {
        ...
        // Hyper mode logic can be added here if needed
      }
      ...
      dispose() {
        ...
        if (this.renderer) {
          ...
          // Force WebGL context loss to free up the context
          if (this.renderer.domElement) {
            const gl = this.renderer.domElement.getContext('webgl') || this.renderer.domElement.getContext('webgl2');
            if (gl) {
    ...
    (function () {
      ...
      options.isPressed = isPressed; // Pass the current pressed state
  ...
  // Update app's isPressed state without recreating the component
  useEffect(() => {
    if (appRef.current) {
      appRef.current.options.isPressed = isPressed;
    }
  }, [isPressed]);
